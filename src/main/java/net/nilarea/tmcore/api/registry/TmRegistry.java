...
import net.nilarea.tmcore.mixin.minecraft.MappedRegistryAccess;
import net.nilarea.tmcore.mixin.minecraft.ResourceKeyAccessor;
...
public class TmRegistry<T> extends MappedRegistry<T> {
    ...
    public TmRegistry(Identifier registryName) {
        this(ResourceKeyAccessor.callCreate(TmRegistries.ROOT_REGISTRY_NAME, registryName));
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    public TmRegistry(ResourceKey<? extends Registry<T>> registryKey) {
        super(registryKey, Lifecycle.stable());
        if (!registryKey.identifier().equals(TmRegistries.ROOT_REGISTRY_NAME)) {
            TmRegistries.ROOT.register((ResourceKey) registryKey, this, RegistrationInfo.BUILT_IN);
        }
    }
    ...
    private boolean checkActiveModContainerIsOwner() {
        ...
        return container != null && (container.getModId().equals(this.key().identifier().getNamespace()) ||
                container.getModId().equals(TechnoMatrixCore.MOD_ID) ||
    ...
    public <V extends T> V register(ResourceKey<T> key, V value) {
        if (containsKey(key)) {
            throw new IllegalStateException(
                    "[TmRegistry] registrate %s contains key %s already".formatted(key().identifier(), key.identifier()));
        }
    ...
    public <V extends T> V replace(ResourceKey<T> key, V value) {
        if (!containsKey(key)) {
            TechnoMatrixCore.LOGGER.warn("[TmRegistry] couldn't find key {} in registrate {}", key, key().identifier());
        }
    ...
    public StreamCodec<RegistryFriendlyByteBuf, T> streamCodec() {
        return new StreamCodec<>() {
            ...
            public @NotNull T decode(@NotNull RegistryFriendlyByteBuf buffer) {
                return TmRegistry.this.byIdOrThrow(VarInt.read(buffer));
            }

            public void encode(@NotNull RegistryFriendlyByteBuf buffer, @NotNull T value) {
                VarInt.write(buffer, TmRegistry.this.getIdOrThrow(value));
            }
        };
    }
