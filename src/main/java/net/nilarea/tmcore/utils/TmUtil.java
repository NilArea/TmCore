package net.nilarea.tmcore.utils;
...
public class TmUtil {
    ...
    private static final Identifier TEMPLATE_LOCATION = Identifier.fromNamespaceAndPath(TechnoMatrixCore.MOD_ID, "");
    ...
    public static String appendIdString(String id) {
        return id.indexOf(':') == -1 ? (TechnoMatrixCore.MOD_ID + ":" + id) : id;
    }

    public static Identifier appendId(String id) {
        String[] strings = new String[] { TechnoMatrixCore.MOD_ID, id };
        int i = id.indexOf(':');
        if (i >= 0) {
